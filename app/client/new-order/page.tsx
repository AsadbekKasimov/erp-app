...
export default function ClientNewOrderPage() {
  ...
  const [items, setItems] = useState<Record<string, { qty: string; due_date: string; notes: string }>>({});
  ...
  return (
    <ClientShell>
      <div style={{ maxWidth: 1500 }}>
        <h1 style={{ fontSize: 22, fontWeight: 700, color: "#111827", marginBottom: 24 }}>{t.newOrderTitle}</h1>
              ...
              <div key={cat} style={{ background: "#fff", border: "1px solid #E5E7EB", borderRadius: 14, overflow: "hidden", marginBottom: 14 }}>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead><tr style={{ background: "#FAFAFA" }}>
                    ...
                    <th style={{ ...th, width: 90 }}>{t.qty}</th>
                    <th style={th}>{t.noteLabel}</th>
                  ...
                  <tbody>
                    {prods.map((p: any) => {
                      ...
                      const active = qty > 0;
                      const missingDt = active && !v.due_date && !!error;
                      return (
                        <tr key={p.id} style={{ borderTop: "1px solid #F3F4F6", background: active ? "#F0FDF4" : "transparent", transition: "background 0.15s" }}>
                          ...
                          <td style={td}>
                            <input type="number" min="0" placeholder="0" value={v.qty}
                              onChange={e => { setItems({ ...items, [p.id]: { ...v, qty: e.target.value } }); setError(""); }}
                              style={{ width: 120, padding: "6px 8px", border: "1px solid #E5E7EB", borderRadius: 8, fontSize: 13, textAlign: "center" }} />
                          </td>
                          
                          <td style={td}>
                            <input type="text" placeholder="..." value={v.notes}
                              onChange={e => setItems({ ...items, [p.id]: { ...v, notes: e.target.value } })}
                              style={{ width: 100, padding: "6px 8px", border: "1px solid #E5E7EB", borderRadius: 8, fontSize: 12 }} />
                          </td>
                          <td style={{ ...td, fontWeight: 700, color: active ? "#065F46" : "#9CA3AF" }}>
        ...
        <button onClick={saveOrder} disabled={saving || (mode === "catalog" && products.length === 0)}
          style={{ width: "100%", padding: 14, background: saving ? "#9CA3AF" : "linear-gradient(135deg, #4F46E5, #7C3AED)", color: "#fff", border: "none", borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: saving ? "not-allowed" : "pointer", boxShadow: saving ? "none" : "0 4px 15px rgba(79,70,229,0.35)" }}>
